...
#[cfg(test)]
mod tests {
    use crate::{crypto::ecdh::ecdh_key, utils::get_random_fe_scalar};
    ...
    fn test_ecdh_secret() {
        ...
        let secret_key1 = get_random_fe_scalar();
        let pub_key1 = g.operate_with_self(secret_key1.representative());

        let secret_key2 = get_random_fe_scalar();
        let pub_key2 = g.operate_with_self(secret_key2.representative());

        assert_eq!(
            ecdh_key(&secret_key1, &pub_key2.to_affine().x()).unwrap(),
            ecdh_key(&secret_key2, &pub_key1.to_affine().x()).unwrap()
        );
    }
}
