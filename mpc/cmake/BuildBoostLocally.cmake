...
if (WIN32)
    ExternalProject_Add(external_boost
            ...
            --with-libraries=context,fiber,log,filesystem,system,thread,program_options,json,atomic,chrono,date_time
            --prefix=${Boost_INSTALL}
            BUILD_COMMAND
            ./b2 install --prefix=${Boost_INSTALL} toolset=gcc address-model=64 link=static variant=${Boost_BUILD_TYPE} ${Boost_SANITIZER_FLAGS} threading=multi -j 10 define=BOOST_ERROR_CODE_HEADER_ONLY define=BOOST_LOG_USE_NATIVE_SYSLOG
            INSTALL_COMMAND ""
...
add_library(boost::log_setup STATIC IMPORTED GLOBAL)
if (WIN32)
...
if (WIN32)
    file(GLOB BOOST_ATOMIC_FILENAME ${Boost_LIB_DIR}/libboost_atomic-mgw*-mt*.a)
    set_property(TARGET boost::atomic PROPERTY IMPORTED_LOCATION ${BOOST_ATOMIC_FILENAME})
else ()
    set_property(TARGET boost::atomic PROPERTY IMPORTED_LOCATION ${Boost_LIB_DIR}/libboost_atomic.a)
endif ()
set_property(TARGET boost::atomic PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${Boost_INCLUDE_DIRS})
add_dependencies(boost::atomic external_boost)
add_library(Boost::atomic ALIAS boost::atomic)

add_library(boost::chrono STATIC IMPORTED GLOBAL)
if (WIN32)
    file(GLOB BOOST_CHRONO_FILENAME ${Boost_LIB_DIR}/libboost_chrono-mgw*-mt*.a)
    set_property(TARGET boost::chrono PROPERTY IMPORTED_LOCATION ${BOOST_CHRONO_FILENAME})
else ()
    set_property(TARGET boost::chrono PROPERTY IMPORTED_LOCATION ${Boost_LIB_DIR}/libboost_chrono.a)
endif ()
set_property(TARGET boost::chrono PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${Boost_INCLUDE_DIRS})
add_dependencies(boost::chrono external_boost)
add_library(Boost::chrono ALIAS boost::chrono)

add_library(boost::date_time STATIC IMPORTED GLOBAL)
if (WIN32)
    file(GLOB BOOST_DATE_TIME_FILENAME ${Boost_LIB_DIR}/libboost_date_time-mgw*-mt*.a)
    set_property(TARGET boost::date_time PROPERTY IMPORTED_LOCATION ${BOOST_DATE_TIME_FILENAME})
else ()
    set_property(TARGET boost::date_time PROPERTY IMPORTED_LOCATION ${Boost_LIB_DIR}/libboost_date_time.a)
endif ()
set_property(TARGET boost::date_time PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${Boost_INCLUDE_DIRS})
add_dependencies(boost::date_time external_boost)
add_library(Boost::date_time ALIAS boost::date_time)
